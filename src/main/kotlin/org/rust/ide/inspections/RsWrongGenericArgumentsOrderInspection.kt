    ...
    override fun buildVisitor(holder: RsProblemsHolder, isOnTheFly: Boolean): RsVisitor =
        object : RsVisitor() {
            override fun visitBaseType(type: RsBaseType) {
                ...
                checkGenericArguments(holder, type)
            }

            override fun visitTraitRef(trait: RsTraitRef) {
                if (!isPathValid(trait.path)) return
                checkGenericArguments(holder, trait)
            }

            override fun visitCallExpr(element: RsCallExpr) = checkGenericArguments(holder, element)
            override fun visitMethodCall(element: RsMethodCall) = checkGenericArguments(holder, element)
        }
